...
func TestSocketProxyClient(t *testing.T) {
	...
	restoredStateHash, err := proxy.Restore(snapshot)
	if err != nil {
		t.Fatalf("Error restoring snapshot: %v", err)
	}

	if !reflect.DeepEqual(restoredStateHash, expectedStateHash) {
		t.Fatalf("Restored stateHash should be %v, not %v", expectedStateHash, restoredStateHash)
	}
}
